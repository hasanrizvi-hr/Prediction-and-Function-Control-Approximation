...
class TDAgent(BaseAgent):
    ...
    def agent_step(self, reward, state):
        ...
        ### START CODE HERE ###
        self.weights[(self.last_state-1)//int(self.num_states/self.num_groups) ] += self.step_size*(
            reward + self.discount_factor*np.dot(self.weights,current_state_feature) - 
            np.dot(self.weights,last_state_feature) )
    ...
    def agent_end(self, reward):
        ...
        ### START CODE HERE ###
        self.weights[(self.last_state-1)//int(self.num_states/self.num_groups) ] += self.step_size*(
            reward - np.dot(self.weights,last_state_feature) )
        ### END CODE HERE ###
        return
